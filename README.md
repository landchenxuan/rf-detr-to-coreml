...
The current tested compatibility baseline is RF-DETR 1.10.1, Core ML Tools 9.0,
and torch 2.7.0.
...
`torch`, `coremltools`, and `rfdetr`; the pinned environment also includes
`onnx` and `onnxruntime` and requires Python 3.12 or newer because of SciPy 1.18.
...
The package targets released `coremltools` 9.0 and `rfdetr` 1.10.1. Deprecated
